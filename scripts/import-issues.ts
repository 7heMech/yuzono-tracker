 ...
 */
function causeOf(l: string[], problem: string) {
...
function stageOf(problem: string, cause: string) {
  const t = problem.toLowerCase();
...
const out: string[] = [
  ...
  `INSERT INTO users (discord_id, username, account_created_at, is_maintainer)
     VALUES ('0', 'imported-from-github', 0, 0);`,
];
...
for (const iss of issues) {
  ...
  // Respect the partial unique index: only one open row per source+kind.
  const dedupeKey = `${sourceId ?? 'req:' + norm(headOf(iss.title))}|${kind}`;
  ...
  out.push(
    `INSERT INTO reports (kind, source_id, proposed_name, proposed_url, lang, nsfw, stage, cause, title, status, reporter_id, github_issue, votes, created_at, updated_at, status_changed_at) VALUES (` +
      [
        esc(kind),
        nul(sourceId ?? null),
